...
import { useAddBundle } from "@/hooks/use-bundles";
...
const AddBundle = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({ title: error.message || "Failed to add bundle", variant: "destructive" });
    }
  };
    ...
    <div>
      <div className="bg-muted/40 px-6 py-3 border-b border-border">
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          <button onClick={() => navigate("/dashboard/k-plans")} className="hover:text-foreground transition-colors">Bundles</button>
        ...
        <div className="bg-card rounded-xl border shadow-sm max-w-3xl mx-auto">
          <div className="px-8 pt-6">
            <div className="inline-flex">
              <span className="text-sm font-semibold text-primary border-b-[3px] border-primary pb-3 px-1">Bundle Information</span>
            </div>
          ...
          <form onSubmit={handleSubmit} className="px-8 py-8">
            ...
            <div className="max-w-md mx-auto space-y-5">
              <div className="space-y-2">
                ...
                <Input id="name" placeholder="Bundle Display Name" value={name} onChange={(e) => setName(e.target.value)} maxLength={100} />
              </div>
              <div className="space-y-2">
              ...
              <div className="pt-4">
                <Button type="submit" className="w-full uppercase font-bold tracking-wide" disabled={addBundle.isPending}>
                  {addBundle.isPending ? "Adding..." : "Add Bundle"}
                </Button>
              </div>
            </div>
          </form>
